...
PluginSystem.onServerStopped(async server => {
    if(await new Promise(r => {
        async () => {
            ...
            console.log('test');
            const restart = await pathExists(join(server.path, 'restart.tmp'));
            ...
            new Map<string, string[][]>(Object.entries(data)).forEach((structure, id) => {
                structure.forEach(async (thing, index) => {
                    ...
                    (parsed as any).value.structure.value.palette.value.default.value.block_position_data.value["0"]
